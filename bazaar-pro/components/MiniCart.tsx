...
import {H4, H5, H6, Paragraph, Tiny} from './Typography';
      ...
      <Box overflow='auto' height={`calc(100vh - ${!!cartList.length ? '80px - 3.25rem' : '0px'})`}>
        ...
        {cartList.map((item: CartItem) => (
          ...
          >
            ...
            <Link href={`/product/${item.pid}___${item.skuid ?? item.pid}`} legacyBehavior>
              <a>
                <Avatar
                  alt={item.title}
            ...
            >
              ...
              <Tiny color='grey.600'>
                {currency(item.onSale === 'true' ? item.sale_price : item.price)} x {item.qty}
              </Tiny>

              <FlexBox gap={1} fontWeight={600} fontSize='14px' color='primary.main' mt={0.5}>
                {item.onSale === 'true' ? (
                  <>
                    {currency(item.qty * item.sale_price)}{' '}
                    <H6 color='grey.600'>
                      <del>{currency(item.qty * item.price)}</del>
                    </H6>
                  </>
                ) : (
