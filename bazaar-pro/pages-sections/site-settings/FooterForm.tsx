...
const FooterForm: FC = () => {
  ...
  const handleFormSubmit = async (values) => {
    console.log(`[${new Date().toLocaleString()}] [INFO] FooterForm.tsx > handleFormSubmit : values from form submission `, values);
  };
      ...
      {({ values, handleChange, handleBlur, handleSubmit, setFieldValue }) => (
        <form onSubmit={handleSubmit}>
          <Grid container spacing={3}>
            <Grid item xs={12}>
              <DropZone
                onChange={(files) => console.log(`[${new Date().toLocaleString()}] [INFO] FooterForm.tsx > DropZone.onChange : files from change event  `, files)}
