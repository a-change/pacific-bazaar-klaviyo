...
const GeneralForm: FC = () => {
  ...
  const handleFormSubmit = async (values) => {
    console.log(`[${new Date().toLocaleString()}] [INFO] GeneralForm.tsx > handleFormSubmit : values from form submission `, values);
  };
      ...
      {({
        ...
        <form onSubmit={handleSubmit} encType="multipart/form-data">
          <Grid container spacing={3}>
            <Grid item xs={12}>
              <DropZone
                onChange={(files) => console.log(`[${new Date().toLocaleString()}] [INFO] GeneralForm.tsx > DropZone.onChange : files from change event  `, files)}
